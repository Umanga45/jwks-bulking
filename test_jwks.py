...
def test_get_jwks_with_keys(client):
    with get_db_connection() as conn:
        conn.execute('DELETE FROM keys')  # Ensure we start with an empty keys table
        ...
        conn.execute('INSERT INTO keys (kid, key, exp) VALUES (?, ?, ?)', (kid, valid_key, int(time.time()) + 3600))
        conn.commit()
...
def test_get_jwks_with_no_keys(client):
    with get_db_connection() as conn:
        conn.execute('DELETE FROM keys')
        conn.commit()
    response = client.get('/.well-known/jwks.json')
